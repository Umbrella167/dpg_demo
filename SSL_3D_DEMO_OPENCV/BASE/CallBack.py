...
def left_mouse_drag_callback(obj):
    ...
    if data.PARAM.mouse.click_obj == "canvas":
        pass
    else:
...
def mouse_move_callback():
    ...
    if dpg.is_mouse_button_down(dpg.mvMouseButton_Middle):
        click_name = data.PARAM.mouse.click_obj
        center_x = param.canvas.width / 2
        center_y = param.canvas.height / 2
        translation_x ,translation_y,_ = param.canvas.translation
        ...
        if data.PARAM.mouse.click_obj == "canvas":
            ...
            param.canvas.translation = [translation_x+move_x, translation_y+move_y, 0]
            param.canvas.translation_matrix = dpg.create_translation_matrix(param.canvas.translation)
        else:
            pass

def window_resize_callback():
    param.canvas.translation = [param.canvas.width /2,param.canvas.height /2,0]
    param.canvas.translation_matrix = dpg.create_translation_matrix(param.canvas.translation)
...
def mouse_wheel_handler(sender, app_data):
    ...
    dpg.focus_item("canvas")
    if mouse_x < param.canvas.width and mouse_y < param.canvas.height:
        # 计算当前鼠标在世界坐标系中的位置
        world_mouse_x = (mouse_x - param.canvas.translation[0]) / data.PARAM.mouse.scale
        world_mouse_y = (mouse_y - param.canvas.translation[1]) / data.PARAM.mouse.scale
        # 根据滚轮值调整缩放比例
        ...
        param.canvas.scales = [data.PARAM.mouse.scale, data.PARAM.mouse.scale, 1]
        param.canvas.scale_matrix = dpg.create_scale_matrix(param.canvas.scales)
        # 计算新的平移量，以使缩放后的鼠标位置与缩放前相同
        new_translation_x = mouse_x - world_mouse_x * param.canvas.scales[0]
        new_translation_y = mouse_y - world_mouse_y * param.canvas.scales[1]
        param.canvas.translation = [new_translation_x, new_translation_y, 0]
        param.canvas.translation_matrix = dpg.create_translation_matrix(param.canvas.translation)
